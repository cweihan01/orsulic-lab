...
class FeatureTCGAViewSet(viewsets.ModelViewSet):
    queryset = Feature_TCGA.objects.all()
    serializer_class = FeatureTCGASerializer

    # Get the categories with /api/features/categories/
    @action(detail=False, methods=['get'])
    def categories(self, request):
        categories = Feature_TCGA.objects.values_list('category', flat=True).distinct()
        return Response({'categories': list(categories)})
    ...
    def subcategories(self, request):
        ...
        subcategories = Feature_TCGA.objects.filter(category__in=categories)\
            .values_list('sub_category', flat=True)\
            .distinct()
...
class NuclearTCGAViewSet(viewsets.ModelViewSet):
    queryset = Nuclear_TCGA.objects.all()
    serializer_class = NuclearTCGASerializer


class MolecularTCGAViewSet(viewsets.ModelViewSet):
    queryset = Molecular_TCGA.objects.all()
    serializer_class = MolecularTCGASerializer


class ClinicalTCGAViewSet(viewsets.ModelViewSet):
    queryset = Clinical_TCGA.objects.all()
    serializer_class = ClinicalTCGASerializer


class FracLacTCGAViewSet(viewsets.ModelViewSet):
    queryset = FracLac_TCGA.objects.all()
...
class ScatterTCGAView(APIView):
    def post(self, request, *args, **kwargs):
        try:
            ...
            # Fetch Feature objects
            try:
                feature1 = Feature_TCGA.objects.get(name=f1_name)
            except Feature_TCGA.DoesNotExist:
                return Response({"error": f"Feature '{f1_name}' not found."}, status=status.HTTP_404_NOT_FOUND)

            try:
                feature2 = Feature_TCGA.objects.get(name=f2_name)  # Fetch single feature
            except Feature_TCGA.DoesNotExist:
                return Response({"error": f"Feature '{f2_name}' not found."}, status=status.HTTP_404_NOT_FOUND)
            ...
            # Query TCGA tables using the retrieved Features
            if db1_name == "Nuclear":
                f1_data = Nuclear_TCGA.objects.filter(feature=feature1)
            elif db1_name == "Molecular":
                f1_data = Molecular_TCGA.objects.filter(feature=feature1)
            elif db1_name == "Clinical":
            ...
            if db2_name == "Nuclear":
                ...
                f2_data = Molecular_TCGA.objects.filter(feature=feature2)
            elif db2_name == "Clinical":
            ...
            if not f1_data or not f2_data:
                # print(f1_data.values_list())
                # print(f2_data)
                return Response({"error": "No TCGA data found for the specified features."}, status=status.HTTP_404_NOT_FOUND)

            # Convert TCGA data into dataframes
            f1_queryset = f1_data.values_list()
            f2_queryset = f2_data.values_list()
            f1_df = pd.DataFrame(list(f1_queryset), columns=["Feature", *PATIENTS])
            f2_df = pd.DataFrame(list(f2_queryset), columns=["Feature", *PATIENTS])
